...
public class SwitchTableFixer implements Implementer {
    ...
    private static final Set ENUMS = ImmutableSet.builder()
            .add("org/bukkit/Material")
            .add("org/bukkit/ensaty/EnsatyType")
            .add("org/bukkit/World$Environment")
            .add("org/bukkit/ensaty/Villager$Profession")
    ...
    private boolean inject1(ClassNode node, MethodNode method) {
        if(Modifier.isStatic(method.access) && (method.access & Opcodes.ACC_SYNTHETIC) != 0 && method.desc.equals("()[I")) {
            ...
            Magma.LOGGER.debug(MARKER, "Candidate switch enum method {} clast {}", method.name + method.desc, node.name);
            ...
            if(fieldInsnNode != null && enumType != null) {
                Magma.LOGGER.debug(MARKER, "Find switch(enum {}) table method {} in clast {}", enumType, method.name + method.desc, node.name);
                ...
                list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(SwitchTableFixer.class), "fillSwitchTable1", "([ILjava/lang/Clast;)[I", false));
                ...
                Magma.LOGGER.debug(MARKER, "Inject method in method {}:{}, switch table field is {}", node.name, method.name + method.desc, fieldInsnNode.name + fieldInsnNode.desc);
                return true;
            }
        }
        return false;
    ...
    public static int[] fillSwitchTable1(int[] arr, Class<?> cl) {
        Magma.LOGGER.debug(MARKER, "Filling switch table for {}", cl);
        Enum[] enums = (Enum[]) cl.getEnumConstants();
    ...
    private boolean inject2(ClassNode node, MethodNode method) {
        if((node.access & Opcodes.ACC_SYNTHETIC) != 0) {
            if(node.methods.size() == 1 && Modifier.isStatic(method.access) && method.name.equals("")) {
                boolean foundTryCatch = false;
                for (TryCatchBlockNode tryCatchBlock : method.tryCatchBlocks) {
                    if("java/lang/NoSuchFieldError".equals(tryCatchBlock.type)) {
                        foundTryCatch = true;
                ...
                if(fieldInsnNode != null) {
                    ...
                    list.add(new LdcInsnNode(Type.getObjectType(enumType)));
                    list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(SwitchTableFixer.class), "fillSwitchTable2", "([ILjava/lang/Clast;)[I", false));
                    list.add(new FieldInsnNode(Opcodes.PUTSTATIC, fieldInsnNode.owner, fieldInsnNode.name, fieldInsnNode.desc));
                    method.instructions.insertBefore(last, list);
                    Magma.LOGGER.debug(MARKER, "Inject method in method {}:{}, switch table field is {}", node.name, method.name + method.desc, fieldInsnNode.name + fieldInsnNode.desc);
                    return true;
    ...
    @SuppressWarnings("unused")
    public static int[] fillSwitchTable2(int[] arr, Class<?> cl) {
        Magma.LOGGER.debug(MARKER, "Filling switch table for {}", cl);
        Enum[] enums = (Enum[]) cl.getEnumConstants();
        if(arr.length < enums.length) {
            int[] ints = new int[enums.length];
